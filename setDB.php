...
try {
    ...
    $stationsCsvPath = "./csv/Fahrradstationen_Frankfurt.csv";
    $routesCsvPath = "./csv/Fahrradbuchungen_Frankfurt.csv";

    if (!file_exists($stationsCsvPath)) {
        die("Stations CSV file does not exist at path: $stationsCsvPath");
    }
    if (!file_exists($routesCsvPath)) {
        die("Routes CSV file does not exist at path: $routesCsvPath");
    }
